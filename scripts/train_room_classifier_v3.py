...
REPORT_PATH = (
    PROJECT_ROOT
    / "outputs"
    / "room_classifier_v3_report.txt"
)

MODEL_PATH = (
    MODEL_DIRECTORY
    / "room_classifier_v3.pkl"
)

LABEL_ENCODER_PATH = (
    MODEL_DIRECTORY
    / "room_label_encoder_v3.pkl"
)

FEATURE_COLUMNS_PATH = (
    MODEL_DIRECTORY
    / "room_feature_columns_v3.pkl"
)

TEST_PREDICTIONS_PATH = (
    PROJECT_ROOT
    / "outputs"
    / "room_classifier_v3_test_predictions.csv"
)
...
def save_confusion_matrix(
    ...
    axis.set_title(
        "Random Forest Room Classifier V3\n"
        "Confusion Matrix"
    )
    ...
    output_path = (
        CHART_DIRECTORY
        / "random_forest_v3_confusion_matrix.png"
    )
...
def save_feature_importance_chart(
    ...
    output_path = (
        CHART_DIRECTORY
        / "random_forest_v3_feature_importance.png"
    )

    importance_csv_path = (
        CHART_DIRECTORY
        / "random_forest_v3_feature_importance.csv"
    )
...
def save_test_predictions(
    ...
    result["actual_room_type"] = actual_labels
    result["predicted_room_type"] = predicted_labels
    ...
    if probability_count != len(class_names):
        print()
        print(
            "Warning: probability columns do not match "
            "the total encoded classes."
        )
        print(
            f"Model probability columns : {probability_count}"
        )
        print(
            f"Total encoded classes     : {len(class_names)}"
        )

    for probability_index in range(probability_count):
    ...
    missing_class_names = class_names[
        probability_count:
    ]
    ...
    if missing_class_names:
        print()
        print(
            "Classes absent from training split:",
            list(missing_class_names),
        )
