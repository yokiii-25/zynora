...
def build_local_render_prompt(
    ...
    return (
        f"{subject}, {style} architecture, {materials}, {roof}, "
        f"{lighting}, {surroundings}, {viewpoint}. Preserve the exact "
        "building silhouette, footprint, wall edges, relative heights, "
        "openings, stairs, proportions, orientation, and camera framing "
        "from the reference image. Apply realistic facade materials, "
        "glazing, physically plausible light, reflections, and shadows. "
        "Keep the full building visible. Do not mirror, rotate, crop, "
        "stretch, duplicate, or replace the building. Do not add or remove "
        "floors or major wings. Additional request: "
        f"{custom_instructions}. Professional residential architecture "
        "photograph, highly detailed, clean composition."
    )
...
def generate_local_house_render(
    ...
    try:
        ...
        negative_prompt = (
            "people, person, human, vehicle, car, text, caption, logo, "
            "watermark, dimensions, floor-plan labels, UI, duplicate "
            "building, extra floor, missing floor, distorted walls, bent "
            "lines, warped perspective, fisheye, cropped building, blurry, "
            "low detail, oversaturated, cartoon, illustration"
        )
