# ZYNORA Blender Phase 2.1 — Framing and Façade Correction

This package includes the Phase 2 real FloorPlanJSON renderer plus the first
review correction. Blender reads the
...
Phase 2.1 keeps the exported wall/opening geometry unchanged and corrects the
presentation layer:

- wider front framing so the full façade has breathing room;
- front-centred camera targets instead of plan-centred targets;
- useful three-quarter side views;
- a compact balcony anchored to an upper-floor door;
- shrubs kept clear of doorway openings; and
- slightly stronger daylight contrast in Eevee previews.
...
```text
Y:\YPS-Labs\zynora\outputs\blender\phase2-1-YYYYMMDD-HHMMSS
```
...
## Final-quality test after geometry is approved
...
`render_phase1_test.py`. Phase 2 must run `render_zynora_floorplan.py` through
`run_zynora_floorplan.ps1`.
